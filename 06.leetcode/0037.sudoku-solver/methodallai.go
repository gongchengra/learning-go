...
func tryNext(pos [][]byte, stack *list.List, k int, v []byte) (byte, bool) {
	pos[k] = []byte{v[0]}
	update(pos)
	remain := remove(v, v[0])
	if len(remain) > 0 {
	...
	return v[0], len(remain) > 0
}

func unknowCells(pos [][]byte) []kp {
	var res []kp
	for i, k := range pos {
		if len(k) > 1 {
