...
func main() {
	...
	/*
		start := time.Now()
		fmt.Println("number of primes (non-segmented-method):", len(SieveOfEratosthenes(*primeRange)))
		fmt.Println("seconds it took:", time.Since(start))
		start = time.Now()
		fmt.Println("number of primes (segmented-method):", len(Primes(*primeRange)))
		fmt.Println("seconds it took:", time.Since(start))
	*/
	fmt.Println(Primes(*primeRange))
	if *memprofile != "" {
